...
const STATES: Record<StepKey, FigState> = {
  ...
  province: { ...base, hist: true, colour: 'split', squares: 0, mine: 0, level: 1, median: true, you: true, ghost: true },
  municipality: { ...base, hist: true, colour: 'split', squares: 0, mine: 0, level: 2, median: true, you: true, ghost: true },
}
...
export default function Figure({ step, levels, you, below, guess, income, guessValue, reduced, compact, number }: FigureProps) {
  ...
  if (step === 'guess') {
    readout = <Read kind="guess" label="Creías" value={String(guess)} />
  } else if (step === 'you' || step === 'hist' || step === 'lines') {
    readout = (
      <>
        <Read kind="guess" label="Creías" value={String(guess)} />
        <Read kind="you" label="Eres" value={String(shownYou)} />
      </>
    )
...
function captionFor(step: StepKey, levels: Level[]): string {
  switch (step) {
    ...
    case 'order':
      return 'Ordenadas según los ingresos de su hogar'
    case 'guess':
    ...
    case 'lines':
      return 'Tu estimación y tu renta'
    case 'province':
...
function Plot({ hist, st, levels, income, guessValue, reduced, compact }: PlotProps) {
  ...
  const wanted = [
    { id: 'median', x: mx, text: `Mediana: ${euro(level.median)}`, on: st.median },
    { id: 'guess', x: gx, text: `Creías: ${euro(guessValue)}`, on: st.guessLine },
    { id: 'you', x: ux, text: beyond ? `Tú: ${euro(income)} →` : `Tú: ${euro(income)}`, on: st.you },
  ]
