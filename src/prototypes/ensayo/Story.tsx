...
import type { Level } from '@/prototypes/shared/useLevels'
import { displayPercentile, euro, perceptionGap } from '@/prototypes/shared/format'
import Figure, { STEP_KEYS, type StepKey } from './Figure'
import { XMAX, binText, cx, levelPhrase, sentenceFor } from './copy'
...
export default function Story({ levels, rawNational, guess, income, guessValue }: StoryProps) {
  ...
  const gapSentence = gap.diff === 0 ? `Creías estar en el ${guess}: lo clavaste.` : gap.sentence
...
function stepCopy({ levels, rawNational, you, below, guess, income, guessValue, gapSentence, diff, right }: CopyInput): StepCopy[] {
  ...
  return [
    ...
    {
      key: 'order',
      title: 'Ahora las ordenamos según los ingresos de su hogar, de menos a más.',
      body: [
        'Arriba a la izquierda, la persona que vive en el hogar con menos ingresos; abajo a la derecha, la que vive en el que más.',
        'Los ingresos se cuentan por unidad de consumo, para comparar hogares de distinto tamaño.',
      ],
    },
    {
      key: 'guess',
    ...
    },
    {
      ...
      body: [
        you <= 1
          ...
          : `${you} de cada 100 personas viven en hogares con menos ingresos que el tuyo: tú eres el cuadrado azul.`,
        gapSentence,
      ],
    },
    {
    ...
    },
    {
      ...
      body: [
        ...
        `La línea de puntos es la mediana: la mitad de la población vive en hogares con menos de ${euro(national.median)} por unidad de consumo.`,
      ],
    },
    {
      key: 'lines',
      ...
      body: [
        `Donde creías estar, con ${guess} ${guess === 1 ? 'persona' : 'personas'} por debajo, se vive con unos ${euro(guessValue)} por unidad de consumo. Tu hogar tiene ${euro(income)}${beyond ? ', más allá del borde del gráfico' : ''}.`,
        left,
      ],
    },
    {
    ...
    },
    {
      ...
      body: [
        ...
        `En España quedaban ${you} de cada 100 por debajo de ti; en ${provPlace}, ${provincial.percentile}; aquí, ${municipal.percentile}.`,
      ],
    },
  ]
}
...
function describe(
  ...
  switch (key) {
    ...
    case 'you':
      return below === 0
        ? 'Figura: tu cuadrado, en azul, es el primero de la cuadrícula: nadie de los cien vive en un hogar con menos ingresos que el tuyo.'
        : `Figura: tu cuadrado, en azul, con ${below} cuadrados más oscuros antes que él: personas que viven en hogares con menos ingresos que el tuyo. Los ${99 - below} restantes, más claros, tienen más.`
    case 'hist':
    ...
    case 'lines':
      return `Figura: una línea discontinua marca tu estimación, ${euro(guessValue)}, y una línea azul tu renta, ${euro(income)}. A la izquierda de la línea azul queda el ${you} % de la población.`
    case 'province':
      return `Figura: la curva pasa a ser la de la provincia de ${levels[1].place}. Allí quedan ${levels[1].percentile} de cada 100 por debajo de ti.`
    case 'municipality':
      return `Figura: la curva pasa a ser la del municipio de ${levels[2].place}. Allí quedan ${levels[2].percentile} de cada 100 por debajo de ti.`
  }
}
