...
export interface Flow {
  ...
  error: string | null
  /** runs the real calculation (same code as the live app); never logs answers */
  calculate: () => Promise<CalculatedResults | null>
  /** back to editing; the answers are kept */
  reset: () => void
}

// Answers + the real calculation. The prototypes never write to the research
// sheet or load analytics: they only read the public data files.
export function useFlow(options: { initial?: Partial<Answers>; minLoadingMs?: number } = {}): Flow {
  const { initial, minLoadingMs = 900 } = options
  ...
  }, [canCalculate, answers, municipalities, minLoadingMs])
