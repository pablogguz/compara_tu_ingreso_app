...
methodology/code/   (R pipeline, run from methodology/ by run_pipeline.sh)
        │
        ├── 0d. nowcast.r               # national nowcast factor, ADRH year → target year (AEAT tax data); sets BASE/TARGET years
...
4. **Nowcast to 2025:** the indicator is AEAT household income excluding capital gains, net of income tax, per person — the same tax records the ADRH is built from, published about six months after the year. ρ = Σ ADRH national growth / Σ AEAT growth over the years both cover (2016–2023, currently 0.933); growth in each later year = ρ × AEAT growth (2024: 5.8%, 2025: 4.2%), chained into one factor (1.102). A pseudo-real-time backtest (ρ from the ADRH up to B, predict B+2) misses the ADRH level by 0.5 pp on average, against 1.7 pp one year ahead for the earlier ECV-based method. Every income is scaled by that factor (GB2 `b → k·b`, log-normal `μ → μ + ln k`), and the municipal income stat likewise. The app asks users for their 2025 income.
5. Outputs are written as FST and converted to Arrow by [scripts/convert-data.R](scripts/convert-data.R). The municipality lookup only contains municipalities with an estimated distribution (81 ADRH municipalities without income data are excluded). `net_income_equiv_is_imputed = 1` exactly where the ADRH publishes no income per consumption unit (1,324 municipalities in the lookup, shown as "(2025, estimada)"); 1,326 municipalities are built only from fallback tracts.
...
- **Intro** (`#introduccion`): a screen of its own with "Si pusiéramos en fila…", a "Comenzar" button and "Las cuentas se hacen en tu navegador". The questions section is not rendered until "Comenzar"; it then scrolls in, focuses the first field and takes a full screen (`.stage`, min-height 100svh) while it is being answered, with only a visually hidden heading. The story and summary are numbered 1 and 2. Nothing appears abruptly: a square spinner ("Calculando…", 16 squares in a diagonal wave with a blue one hopping) stays until the percentiles *and* the figures are ready (`Results` calls `onReady`, or the error boundary `onError`); then the answers box and the story rise in (heading, figure, steps, staggered) as the page glides to the story, and the intro, summary blocks and closing line rise into place the first time they are seen (`useReveal` in `hooks.ts`; instant without IntersectionObserver or with reduced motion). The hero title is not `text-wrap: balance`d: with its inline-block words, WebKit sometimes broke it one word per line.
