...
\subsection{Nowcasting to the current year}

The ADRH is published with a lag of roughly two years, so the most recent available edition refers to income year 2023. To compare users against a contemporaneous distribution, I project tract-level incomes forward to the latest year for which aggregate income growth is observed (2024) using the Spanish Living Conditions Survey (\textit{Encuesta de Condiciones de Vida}, ECV, the national component of EU-SILC). The ECV reports mean equivalised income (\textit{renta media por unidad de consumo}) by autonomous community, with survey wave $t+1$ referring to income earned in year $t$. For each autonomous community $c$, I compute the gross growth factor

\begin{equation}
g_c = \frac{\bar{y}^{\,\text{ECV}}_{c,\,2024}}{\bar{y}^{\,\text{ECV}}_{c,\,2023}},
\end{equation}

and scale the income of every tract $j \in c$ by $g_c$. Because multiplying a log-normal variate by a constant shifts its location parameter, $\nu_j \mapsto \nu_j + \ln g_c$, while leaving the shape parameter $\sigma_j$ unchanged, this operation preserves within-tract inequality --- and hence the tract Gini coefficient --- and updates only the level of the distribution. The upscaling factor is applied identically to the income of every census tract and municipality within an autonomous community, so that the entire small-area distribution shifts up while its shape is preserved.

To ensure that the implied national growth matches the aggregate ECV figure exactly, I rescale the community-level factors by a common constant,

\begin{equation}
\tilde{g}_c = k\, g_c, \qquad k = \frac{g_{\text{ES}}}{\sum_c s_c\, g_c},
\end{equation}

where $g_{\text{ES}}$ is the ECV national growth factor and $s_c$ is the share of autonomous community $c$ in aggregate equivalised income, computed from the ADRH. Since the national mean of the mixture grows by the income-weighted average of the community factors, this construction guarantees $\sum_c s_c\, \tilde{g}_c = g_{\text{ES}}$: the national mean is anchored to the ECV national figure while relative differences in regional income growth are retained. In the 2023--2024 application, national mean equivalised income grew by 5.4\%, with (calibrated) community-level factors ranging from $-1.1\%$ (Melilla) to $+8.3\%$ (Cantabria). This adjustment rests on two assumptions: that the year-on-year \emph{growth} of survey-based equivalised income is a good proxy for growth in the administrative (tax-based) measure within each region --- levels differ across the two sources, but growth rates are considerably more comparable --- and that within-area inequality is stable over the one-year horizon.
