...
The ADRH is published with a lag of roughly two years, so the most recent edition refers to income year 2023. To compare users against a contemporaneous distribution, I project incomes forward to 2024 using the Spanish Living Conditions Survey (\textit{Encuesta de Condiciones de Vida}, ECV), the national component of EU-SILC, whose survey wave $t+1$ reports mean equivalised income (\textit{renta media por unidad de consumo}) earned in year $t$.

Let $g^{\text{ADRH}}_t$ and $g^{\text{ECV}}_t$ denote the year-on-year growth of national mean equivalised income in each source, where the ADRH mean is the population-weighted average across the municipalities observed in both years. Survey and administrative growth rates need not coincide, so I measure by how much the ECV over- or understates growth in the administrative data over the years covered by both sources,

\begin{equation}
\rho = \frac{\sum_{t=2016}^{2023} g^{\text{ADRH}}_t}{\sum_{t=2016}^{2023} g^{\text{ECV}}_t},
\end{equation}

and nowcast income growth in 2024 as

\begin{equation}
\hat{g}_{2024} = \rho \, g^{\text{ECV}}_{2024}.
\end{equation}

Over 2016--2023 the ECV slightly overstates administrative income growth ($\rho = 0.93$). With ECV growth of 5.4\% in 2024, this implies that equivalised income grew by 5.1\%.

I scale the income of every census tract by $1+\hat{g}_{2024}$. Because multiplying a log-normal variate by a constant shifts its location parameter, $\nu_j \mapsto \nu_j + \ln(1+\hat{g}_{2024})$, while leaving $\sigma_j$ unchanged, the adjustment updates the level of every small-area distribution and preserves its shape, and hence the tract Gini coefficients. The underlying assumption is that incomes grew at a common rate across areas and along the distribution over the one-year horizon.
