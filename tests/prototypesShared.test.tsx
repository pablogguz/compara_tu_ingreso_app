...
import {
  ...
  scaleExplained,
  displayPercentile,
} from '@/prototypes/shared/format'
import { resample, curvePaths, ticks, xAt } from '@/prototypes/shared/chart'
...
describe('prototype formatting', () => {
  ...
  it('talks about people, not households, and caps at 99', () => {
    ...
    expect(headline(1, 'Madrid')).toMatch(/entre el 1 % con menos ingresos de Madrid/)
    expect(outOf100(86, 'España')).toBe('De cada 100 personas en España, 86 viven en hogares con menos ingresos que el tuyo.')
    expect(outOf100(1, 'España')).toMatch(/apenas 1 vive en un hogar/)
    expect(displayPercentile(100)).toBe(99)
  })

  it('describes the perception gap in both directions', () => {
    expect(perceptionGap(86, 62)).toMatchObject({ diff: 24, kind: 'under' })
    expect(perceptionGap(40, 70)).toMatchObject({ diff: -30, kind: 'over' })
    expect(perceptionGap(51, 50).kind).toBe('right')
  })

  it('spells out the OECD scale', () => {
    expect(scaleExplained(1, 0)).toBe('1 unidad de consumo')
    expect(scaleExplained(2, 1)).toBe('1 + 0,5 + 0,3 = 1,8 unidades de consumo')
  })
})

describe('prototype chart geometry', () => {
